...
- Runtime data lives in `%APPDATA%/StickyNotes/`: `settings.json`, `open_notes.json`, one `sticky_note_<id>.json` per note (content stored as **HTML**), `md_custom.css`, `bg/`. `settings.json` may carry a UTF-8 BOM (stripped in Rust) — don't "fix" it.
